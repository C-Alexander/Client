...
public class GameStatusWebSocketListener extends WebSocketAdapter {
    ...
    public boolean onOpen(WebSocket webSocket) {
        Gdx.app.log("Networking", "Websocket connection opened");
        joinGame(webSocket);
    ...
    public boolean onClose(WebSocket webSocket, WebSocketCloseCode code, String reason) {
        Gdx.app.log("Networking", "Websocket connection closed. Reason: " + reason);
        stopKeepAlive();
        if (!endGame) {
            webSocket.connect();
        }
    ...
    public boolean onError(WebSocket webSocket, Throwable error) {
        Gdx.app.error("Networking", "Error in the websocket connection", error);

        return false;
    }

    private void joinGame(WebSocket webSocket) {
        if (!webSocket.isOpen()) Gdx.app.error("Networking", "Tried to join game before websocket was open");
